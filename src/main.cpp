...
#if defined(NDEBUG)
# error "OGC cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "OGC Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // OGC: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// OGC: find block wanted by given orphan block
...
{
    int64_t nSubsidy = (uint64_t)(IPO_SHARE/COIN*CENT)/(365*24*3600/60);
    return nSubsidy + nFees;
}

static const int64_t nTargetTimespan = 16 * 60;  // 16 mins

// OGC: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // OGC: target change every block
    // OGC: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal OGC miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // OGC: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // OGC: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // OGC: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // OGC: track money supply and mint amount info
...
// OGC: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // OGC: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // OGC: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // OGC: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// OGC: attempt to generate suitable proof-of-stake
